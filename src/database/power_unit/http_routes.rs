...
pub async fn insert_power_unit(
    conn: Data<DBPool>,
    id: Identity,
    data: Json<NewPowerUnit>,
) -> HttpResponse {
    let pool = conn.get().expect("Error connecting to pool");
    ...
    match check {
        ...
        (true, _) => match PowerUnit::create(data.0, &pool) {
            Ok(val) => HttpResponse::Ok()
                .content_type("application/json")
                .json(format!("Power unit inserted with id: {}", val.id)),
            Err(er) => ErrorHandler::from(er).get_response(),
        },
    }
}

pub async fn delete_power_unit(
    conn: Data<DBPool>,
    id: Identity,
    data: Json<String>,
) -> HttpResponse {
    ...
    match check {
        ...
        (true, _) => match PowerUnit::delete(data.0, &pool) {
            Ok(_) => HttpResponse::Ok()
                .content_type("application/json")
                .json("Powerunit succesfully deleted"),
            Err(er) => ErrorHandler::from(er).get_response(),
        },
    }
}

pub async fn update_power_unit(
    conn: Data<DBPool>,
    id: Identity,
    data: Json<NewPowerUnit>,
) -> HttpResponse {
    ...
    match check {
        ...
        (false, _) => match PowerUnit::update(&pool, data.0) {
            Ok(val) => HttpResponse::Ok()
                .content_type("application/json")
                .json(format!("Power unit with id: {} updated", val.id)),
            Err(er) => ErrorHandler::from(er).get_response(),
        },
    }
}
