...
pub async fn insert_cooling(
    conn: Data<DBPool>,
    id: Identity,
    data: Json<NewCooling>,
) -> HttpResponse {
    let pool = conn.get().expect("Error connecting to pool");
    ...
    if !check.0 {
        ...
        return match result {
            Ok(val) => HttpResponse::Ok()
                .content_type("application/json")
                .json(format!("Cooling succesfully inserted with id: {}", val.id)),
            Err(er) => ErrorHandler::from(er).get_response(),
        };
    }
}
    ...
    if !check.0 {
        ...
        return match result {
            Ok(_) => HttpResponse::Ok()
                .content_type("application/json")
                .json("Cooling succesfully updated"),
            Err(er) => ErrorHandler::from(er).get_response(),
        };
    }
}
...
pub async fn delete_cooling(conn: Data<DBPool>, id: Identity, data: Json<String>) -> HttpResponse {
    ...
    if !check.0 {
        ...
        return match result {
            Ok(_) => HttpResponse::Ok()
                .content_type("application/json")
                .json("Cooling deleted succesfully"),
            Err(er) => ErrorHandler::from(er).get_response(),
        };
    }
}
