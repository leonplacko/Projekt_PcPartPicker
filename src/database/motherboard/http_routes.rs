...
pub async fn insert_motherboard(
    conn: Data<DBPool>,
    id: Identity,
    data: Json<ExtendMotherboard>,
) -> HttpResponse {
    let pool = conn.get().expect("Error connecting to pool");
    ...
    match check_admin(&id) {
        ...
        (true, _) => match Motherboard::create(data.0, &pool) {
            Ok(val) => HttpResponse::Ok()
                .content_type("application/type")
                .json(format!("Motherboard inserted with id: {}", val.id)),
            Err(er) => ErrorHandler::from(er).get_response(),
        },
    }
}
...
pub async fn delete_motherboard(
    ...
    match check_admin(&id) {
        ...
        (true, _) => match Motherboard::delete(data.0, &pool) {
            Ok(val) => HttpResponse::Ok()
                .content_type("application/type")
                .json(format!("Motherboard deleted, amount: {}", val)),
            Err(er) => ErrorHandler::from(er).get_response(),
        },
    }
}
...
pub async fn update_motherboard(
    ...
    match check_admin(&id) {
        ...
        (true, _) => match Motherboard::update(&pool, data.0) {
            Ok(val) => HttpResponse::Ok()
                .content_type("application/type")
                .json(format!("Motherboard updated with id: {}", val.id)),
            Err(er) => ErrorHandler::from(er).get_response(),
        },
    }
}
