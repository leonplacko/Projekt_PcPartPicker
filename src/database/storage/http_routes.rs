...
pub async fn insert_storage(conn: Data<DBPool>, id: Identity, data: Json<ExtendStorage>) -> HttpResponse{
    let pool = conn.get().expect("Error connecting to pool");

    match check_admin(&id){
        (false, er) => er,
        (true, _) => match Storage::create(data.0, &pool){
            Ok(val) => HttpResponse::Ok().content_type("application/json").json(format!("Storage inserted with id: {}", val.id)),
            Err(er) => ErrorHandler::from(er).get_response()
        }
    }
}
...
pub async fn delete_storage(conn: Data<DBPool>, id: Identity, data: Json<String>) -> HttpResponse{
    ...
    match check_admin(&id){
        ...
        (true, _) => match Storage::delete(data.0, &pool){
            Ok(val) => HttpResponse::Ok().content_type("application/json").json(format!("Storage deleted, amount: {}", val)),
            Err(er) => ErrorHandler::from(er).get_response()
        }
    }
}

pub async fn update_storage(conn: Data<DBPool>, id: Identity, data: Json<NewStorage>) -> HttpResponse{
    let pool = conn.get().expect("Error connecting to pool");

    match check_admin(&id){
        (false, er) => er,
        (true, _) => match Storage::update(&pool, data.0){
            Ok(val) => HttpResponse::Ok().content_type("application/json").json(format!("Strorage updated with id: {}", val.id)),
            Err(er) => ErrorHandler::from(er).get_response()
        }
    }
}